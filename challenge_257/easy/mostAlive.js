...
(() => {
	document.addEventListener("DOMContentLoaded", () => {
		 ...
		 *
		 * name  : president name
		 * birth : president date of birth
		 * death : president date of death
		 */
		class President {
			constructor(name, birth, death) {
				this.name = name;
				this.birth = new Date(birth);
				this.death = death ? new Date(death) : new Date();
			}
		} 
		/**
		 * construct a table of president information
		 * @param String
		 *
		 ...
		 * returns obj {}
		 */
		function makePresidentTable(info) {
			let president = new Map();
			info.split("\n")
			    .map(line => line.split(",").map(item => item.trim()))
			    .forEach(line => {
			    	president.set(line[0].toLowerCase(), new President(line[0], line[1], line[3]));
			    });
			return president;
		} 
		/**
		 * check if a president is alive 
		 * during a given year
		 * @param int, obj {}
		 *
		 * year      : year to be checked
		 * president : president to be checked
		 *
		 * returns boolean
		 */
		function isAlive(year, president) {
			return year >= president.birth.getFullYear() && year <= president.death.getFullYear();
		} 
		/**
		 * check presidents who are still
		 * alive for a given year
		 * @param int, obj {}
		 *
		 * year  : year to be checked
		 * table : lookup table for presidents	
		 *
		 * returns array []
		 */
		function totalAlive(year, table) {
			let alive = [];
			table.forEach(president => {
				if(isAlive(year, president)) alive.push(president.name);
			});
			return alive;
		} 
		/**
		 ...
		 * returns array []
		 */
		function getYearRange(table) {
			let minYear = 0, maxYear = 0;
			table.forEach(president => {
				minYear = Math.min(minYear === 0 ? president.birth.getFullYear() : minYear, president.birth.getFullYear());
				maxYear = Math.max(president.death.getFullYear(), maxYear);
			});
			return [minYear, maxYear];
		} 
		/**
		 * find maximum number of presidents alive during a year
		 * @param int, int, obj {}
		 *
		 * minYear : starting year for the search
		 * maxYear : ending year for the search 
		 * table   : lookup table for presidents	
		 *
		 * return array []
		 */
		function maxAlive(minYear, maxYear, table) {
			let maxAlive = 0;
			for(let i = minYear; i <= maxYear; i++) {
				maxAlive = Math.max(totalAlive(i, table).length, maxAlive);
			}
			return maxAlive;
		} 
		/**
		 * find years with maximum number of 
		 * presidents alive and their names
		 * @param String
		 *
		 * info : president information
		 *
		...
		function mostAliveYear(info) {
			...
			for(let i = minYear; i <= maxYear; i++) {
				...
				if(curAlive.length == mostAlive) {
					allAlives.push([i, curAlive]);
				}
			}
			return allAlives;
		}  
		//challenge input
		let input = `George Washington,  Feb 22 1732,    Westmoreland Co. Va.,   Dec 14 1799,    Mount Vernon Va.
		...
		let result = mostAliveYear(input);
		result.forEach(record => {
			console.log(`Total Alive: %c${record[1].length}, %cYear: %c${record[0]}, %cNames: %c${record[1].join(", ")}`, "color : red;", "", "color : red;", "", "color : orange;");						 
		});
	});
})();		