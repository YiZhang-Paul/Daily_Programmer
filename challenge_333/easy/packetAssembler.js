...
(() => {
	document.addEventListener("DOMContentLoaded", () => {
		/**
		 * check if a message is fully assembled
		 * @param {Array} [message] - message to be checked
		 *
		 * @return {boolean} [test result]
		 */
		function isComplete(message) {
			return message.every(packet => packet !== undefined);
		}
		/**
		 * extract all messages that are fully assembled
		 * @param {Object} [messages] - collection of all messages to be checked
		 *
		 * @return {Array} [messages that are fully assembled] 
		 */
		function getComplete(messages) {
			let completed = [];
			messages.forEach((message, id) => {
				if(isComplete(message)) {
					completed.push([id, message.join("\n")]);
				}
			});
			return completed.sort((a, b) => a[0] - b[0]).map(message => message[1]);
		}
		/**
		 * assembled packets
		 * @param {String} [info] - information of incoming packets
		 *
		...
		function assemblePacket(packets) {
			...
			packets.split("\n").forEach(packet => {
				const [id, index, total] = packet.match(/\d+/g).slice(0, 3).map(Number);
				let message = assembled.has(id) ? assembled.get(id) : new Array(total);
				message[index] = packet.trim();
				assembled.set(id, message);
			});
			return getComplete(assembled).join("\n");
		}
		//default input
		...
		console.log(`%c${assemblePacket(input)}`, "color : orange;");		
		//challenge input
		console.log(`%cChallenge Input: `, "color : red;");
		input = `7469    1   7   believe. Attack ships on fire off the 
