...
namespace vigenèreCipher {
    class Program {
        static void Main(string[] args) {
            ...
            //challenge input
            Console.WriteLine(cipher.EncryptOrDecrypt("THECAKEISALIE"));
            Console.WriteLine(cipher.EncryptOrDecrypt("ZSEFOCKTSDZAK", false));
            //bonus input
            string text = @"The easy challenge today is to implement the famous Vigenère cipher. The Wikipedia article explains well how it works, but here's a short description anyway:
            ...
            //try each word in the text as new key -> answer: "BEGINNING"
            /*
             * decrypted text: 
             * 
             */
            foreach(string key in GetWords(text)) {

                cipher.SetKey(key);
                Console.WriteLine(cipher.EncryptOrDecrypt(message, false) + " Key -> " + key + "\n");
            }
        }
        /// <summary>
        /// retrieve all unique words from text
        /// </summary>
        /// <param name="text">text to examine</param>
        /// <returns>all unique words</returns>
        public static string[] GetWords(string text) {

            var matches = Regex.Matches(text.ToUpper(), "[a-zA-Z]+").Cast<Match>();
            //remove duplicate words
            return new HashSet<string>(matches.Select(match => match.Value)).ToArray();
        }
    }
}